...
export default function BaselinePage() {
  ...
  if (loading) return <div className="py-8 text-sm text-zinc-500">Loading baselines...</div>;

  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-zinc-900">Assumption Locker</h1>
          <p className="mt-1 text-sm text-zinc-500">Manage baselines from SOW/MSA documents</p>
        </div>
        <button
          ...
          {baselines.map((b) => (
            <button
              ...
              className={`w-full rounded-lg border p-3 text-left transition-colors ${
                selected?.id === b.id ? "border-zinc-900 bg-zinc-50" : "border-card-border bg-card-bg hover:border-zinc-400"
              }`}
            >
              <div className="flex items-center justify-between">
