...
export async function processInboundEmail(
  ...
  try {
    ...
    // For transcripts and scope-relevant emails, run scope-analysis inline.
    // The ExtractionJob created above is the "extract candidate changes" path;
    // analyzeScopeFromText is the path that populates /scope with
    // ScopeAnalysis + ScopeAlerts. Without this call, transcripts and emails
    // arrive but /scope stays empty — even when the content is clearly a
    // scope-change signal (e.g. "additional training run planned").
